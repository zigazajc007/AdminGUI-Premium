...
public class Database {
		...
		if (playerHasRank(uuid, name)) {
			if (uuid != null) {
				try {
					...
					Connection conn = AdminGUI.hikari.getConnection();
					conn.createStatement().executeUpdate("UPDATE admingui_ranks SET rank = '" + rank + "', created = '" + date_format.format(until) + "' WHERE uuid = '" + uuid + "';");
					conn.close();
					return true;
				} catch (SQLException ignored) {
				}
			...
			if (uuid != null) {
				try {
					Connection conn = AdminGUI.hikari.getConnection();
					conn.createStatement().executeUpdate("INSERT INTO admingui_ranks(uuid, username, rank) VALUES ('" + uuid + "', '" + name + "', '" + rank + "');");
					conn.close();
					return true;
				} catch (SQLException ignored) {
				}
	...
	public static boolean playerHasRank(UUID uuid, String name) {
		if (uuid != null) {
			String query = "SELECT rank FROM admingui_ranks WHERE uuid = '" + uuid + "' OR username = '" + name + "'";
