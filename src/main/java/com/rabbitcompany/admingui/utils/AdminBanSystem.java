...
public class AdminBanSystem {
    ...
    public static String banPlayer(String uuid_from, String username_from, String uuid_to, String username_to, String reason, String until){
        if(AdminGUI.conn != null && AdminGUI.getInstance().getConf().getBoolean("admin_ban_system_enabled", false)) {
            try {
                ...
                return Message.getMessage(UUID.randomUUID(), "message_player_ban").replace("{player}", username_to).replace("{reason}", reason);
    ...
    public static String mutePlayer(String uuid_from, String username_from, String uuid_to, String username_to, String reason, String until){
        if(AdminGUI.conn != null && AdminGUI.getInstance().getConf().getBoolean("admin_ban_system_enabled", false)) {
            try {
                AdminGUI.mySQL.update("INSERT INTO admin_gui_muted_players(uuid_from, username_from, uuid_to, username_to, reason, until) VALUES ('" + uuid_from + "', '" + username_from + "', '" + uuid_to + "', '" + username_to + "', '" + reason + "', '" + until + "');");
                return Message.getMessage(UUID.randomUUID(), "message_player_mute").replace("{player}", username_to).replace("{reason}", reason);
            } catch (SQLException ignored) {
                return Message.chat("&cSomething went wrong while trying to mute " + username_to + ".");
            }
        }else{
            return Message.chat("&cYou don't have enabled Admin Ban System in config or settled up correctly Mysql database!");
        }
    ...
    public static boolean unMutePlayerName(String player){
        if(AdminGUI.conn != null && AdminGUI.getInstance().getConf().getBoolean("admin_ban_system_enabled", false)) {
            if(isPlayerMuted(player)){
                try {
                    Date until = new Date(System.currentTimeMillis());
                    AdminGUI.mySQL.update("UPDATE admin_gui_muted_players SET until = '" + sdf.format(until) + "' WHERE username_to = '" + player + "';");
                    return true;
                } catch (SQLException ignored) { }
            }
        }
    ...
    public static ArrayList<MutedPlayers> getMutedPlayers(){
        ArrayList<MutedPlayers> mutedPlayers = new ArrayList<>();
        ...
        try {
            AdminGUI.mySQL.query(query, results -> {
                while (results.next()){
                    ...
                    Timestamp now = new Timestamp(System.currentTimeMillis());
                    if(now.before(until)){
                        mutedPlayers.add(new MutedPlayers(results.getString("uuid_from"), results.getString("username_from"), results.getString("uuid_to"), results.getString("username_to"), results.getString("reason"), results.getTimestamp("until"), results.getTimestamp("created")));
                    }
                }
            });
        } catch (SQLException e) {
