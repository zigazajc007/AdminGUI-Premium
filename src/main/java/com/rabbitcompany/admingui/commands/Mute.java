...
public class Mute implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(!(sender instanceof Player)) {
            if(args.length < 3){
                sender.sendMessage(Message.getMessage(UUID.randomUUID(), "prefix") + Message.chat("&cPlease use /mute <player> <time> <reason>"));
            }else{
                ...
                if(NumberUtils.isNumber(AdminUI.stripNonDigits(str_time))){
                    ...
                    if(silence){
                        sender.sendMessage(Message.getMessage(UUID.randomUUID(), "prefix") + AdminBanSystem.mutePlayer("Console", "Console", Bukkit.getOfflinePlayer(str_player).getUniqueId().toString(), str_player, reason.toString(), AdminBanSystem.sdf.format(until)));
                    }else{
                        Bukkit.broadcastMessage(Message.getMessage(UUID.randomUUID(), "prefix") + AdminBanSystem.mutePlayer("Console", "Console", Bukkit.getOfflinePlayer(str_player).getUniqueId().toString(), str_player, reason.toString(), AdminBanSystem.sdf.format(until)));
                    }
        ...
        if(player.hasPermission("admingui.mute")){
            if(args.length < 3){
                ...
                if(NumberUtils.isNumber(AdminUI.stripNonDigits(str_time))){
                    ...
                    if(silence){
                        player.sendMessage(Message.getMessage(player.getUniqueId(), "prefix") + AdminBanSystem.mutePlayer(player.getUniqueId().toString(), player.getName(), Bukkit.getOfflinePlayer(str_player).getUniqueId().toString(), str_player, reason.toString(), AdminBanSystem.sdf.format(until)));
                    }else{
                        Bukkit.broadcastMessage(Message.getMessage(player.getUniqueId(), "prefix") + AdminBanSystem.mutePlayer(player.getUniqueId().toString(), player.getName(), Bukkit.getOfflinePlayer(str_player).getUniqueId().toString(), str_player, reason.toString(), AdminBanSystem.sdf.format(until)));
                    }
